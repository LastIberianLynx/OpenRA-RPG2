...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class CommandBarLogic : ChromeLogic
	{
		...
		//PossessActor:
		bool possessActorDisabled = false;
		int possessActorHighlighted;

		//these ones are not necessary though...
		bool moveUpDisabled = false;
		int moveUpHighlighted;

		public PossessActor PossessActor;
		...
		{
			this.world = world;
			PossessActor = new PossessActor(world, this);
			...
			var possessActorButton = widget.GetOrNull<ButtonWidget>("POSSESS_ACTOR");
			if (possessActorButton != null)
			{
				WidgetUtils.BindButtonIcon(possessActorButton);

				possessActorButton.IsDisabled = () => { UpdateStateIfNecessary(); return possessActorDisabled; };
				possessActorButton.IsHighlighted = () => possessActorHighlighted > 0;
				possessActorButton.OnClick = () =>
				{
					if (highlightOnButtonPress)
						possessActorHighlighted = 2;

					// PerformKeyboardOrderOnSelection(a => new Order("Stop", a, false));
					// PossessActor.
					if(PossessActor.PossessedActor == null)
					PossessActor.PossessSelectedActor(selectedActors[0]);
					else
					PossessActor.UnpossessActor();
	

				};

				possessActorButton.OnKeyPress = ki => { possessActorHighlighted = 2; possessActorButton.OnClick(); };
			}

			var moveUpButton = widget.GetOrNull<ButtonWidget>("MOVE_UP");
			if (moveUpButton != null)
			{
				//KEY IS DISABLED
				WidgetUtils.BindButtonIcon(moveUpButton);

				moveUpButton.IsDisabled = () => { UpdateStateIfNecessary(); return moveUpDisabled; };
				moveUpButton.IsHighlighted = () => moveUpHighlighted > 0;
				...
				moveUpButton.OnKeyPress = ki => { moveUpHighlighted = 2; moveUpButton.OnClick(); };
			}
		...
		{
			if(PossessActor != null) {
				PossessActor.HandleInput();
			}
			...
			// if (possessActorHighlighted > 0)
			// 	possessActorHighlighted--;
